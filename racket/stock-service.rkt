...
(define (json->stock jdata)
	(make-stock (string->number (hash-ref jdata 'id))
			(hash-ref jdata 't)
			(hash-ref jdata 'e)
			(string->number (hash-ref jdata 'l))
			(hash-ref jdata 'lt)
			(hash-ref jdata 'ltt)
			(string->number (hash-ref jdata 'c))
			(string->number (hash-ref jdata 'cp))))
...
(define (get-stock-information/json ticker)
	(string->jsexpr (substring (get-stock-information/string ticker) 3)))
